-- ============================================================================
-- 0017_semi_component_manual_restore_materials.sql
-- Yari mamul manuel stok cikisinda parca recetesine gore hammadde iadesi
-- ============================================================================
...
begin
  ...
  if p_source not in ('manual', 'transfer') then
    raise exception 'Gecersiz kaynak';
  end if;
  ...
  if p_restore_materials and p_type = 'out' and p_source = 'manual' then
    for r in
      select *
      from public.collect_recipe_material_requirements(
        p_variant_id,
        p_qty,
        p_component_id,
        false,
    ...
    loop
      ...
      values (
        ...
        r.qty,
        'manual',
        coalesce(p_note, 'Yari mamul manuel stok duzeltmesinden hammadde geri yukleme'),
        v_user
      );
    end loop;
  end if;
